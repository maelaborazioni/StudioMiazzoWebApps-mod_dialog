...
function setupForm(_aArguments, _sIconStyle, _nDialogWidth, _nDialogHeight) {
	...
	// MAVariazione - use a html area if in web client
	// create a message field that has the height of the dialog
	_oField = _oForm.newField('msgText', application.getApplicationType() == APPLICATION_TYPES.WEB_CLIENT ? JSField.HTML_AREA : JSField.TEXT_AREA, 90, 15, _nDialogWidth - 100, _nDialogHeight - 70);
	_oField.borderType = "EmptyBorder,0,0,0,0";
	// MAVariazione - show scrollbars if necessary
	_oField.scrollbars = SM_SCROLLBAR.HORIZONTAL_SCROLLBAR_NEVER | SM_SCROLLBAR.VERTICAL_SCROLLBAR_AS_NEEDED; //SM_SCROLLBAR.HORIZONTAL_SCROLLBAR_NEVER | SM_SCROLLBAR.VERTICAL_SCROLLBAR_NEVER;
